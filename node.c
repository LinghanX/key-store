...
int main(int argc, char *argv[])
{
	...
	while(1){
        // default incoming_package to avoid corrupted data
		struct info_package *incoming_package = (struct info_package*) malloc(sizeof(struct info_package));
		...
		char* key_buffer = malloc(incoming_package->key_size);
		char* value_buffer = malloc(incoming_package->value_size);
		...
		if(incoming_package->method == GET){
			...
			while(DictSize(d) > 0) {
				...
				struct info_package *request = (struct info_package*) malloc(sizeof(struct info_package));
				request->method = PUT;
				request->key_size = strlen(key);
				request->value_size = strlen(ret);

				char* key_buffer = malloc(request->key_size);
				char* value_buffer = malloc(request->value_size);
