...
struct node_info find_post_node(struct node_info* available_nodes,
    int num_of_nodes,
    char* address){
  printf("Given address is:\n%s\n", address);
  struct node_info node;
  strcpy(node.addr,strtok(address, ":"));
  strcpy(node.service, strtok(NULL, ""));
  printf("Looking for node with addr %s and service %s\n", node.addr, node.service);
  ...
  printf("Match node with index %d, while available_nodes is %d", i, num_of_nodes);
...
int stablize(struct node_info *damaged_node,
    struct node_info *available_nodes,
    int num_of_nodes){
  char addr[256];
  strcpy(addr, damaged_node->addr);
  strcat(addr, ":");
  strcat(addr, damaged_node->service);

  struct node_info next_node = find_post_node(available_nodes, num_of_nodes, addr);
  remove_node(available_nodes, num_of_nodes, damaged_node);
  reset_node(next_node);

  return 0;
}

void print_available_nodes(struct node_info *available_nodes,
    int num_of_nodes){
  for(int i = 0; i < num_of_nodes; i++){
    printf("The available node %d is: %s:%s\n",
        i,
        available_nodes[i].addr,
        available_nodes[i].service);
  }
}
/*
